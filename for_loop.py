...
#             predicted_pairs = get_predicted_pairs(products, pre_dissimilarity_matrix, param2, shingle_size, main_param, param1, gamma, mu)
#             TN, TP, FN, FP, F1, precision, recall = get_final_performance(products, predicted_pairs, true_pairs)
#             results.append((recall, precision, F1))
#         elif main_param_index == 1:
#             predicted_pairs = get_predicted_pairs(products, pre_dissimilarity_matrix, param2, shingle_size, param1, main_param, gamma, mu)
#             TN, TP, FN, FP, F1, precision, recall = get_final_performance(products, predicted_pairs, true_pairs)
#             results.append((recall, precision, F1))
#         else:
#             predicted_pairs = get_predicted_pairs(products, pre_dissimilarity_matrix, main_param, shingle_size, param1, param2, gamma, mu)
#             TN, TP, FN, FP, F1, precision, recall = get_final_performance(products, predicted_pairs, true_pairs)
#             results.append((recall, precision, F1))
#     return zip(*results)
#
...
# plot_subplots(fig_alpha, axes_alpha, alpha_values, beta_threshold_product, 'Alpha Plots', 0, len(beta_values), len(threshold_values))
#
# fig_beta, axes_beta = plt.subplots(len(alpha_values), len(threshold_values), figsize=(15, 15))
# plot_subplots(fig_beta, axes_beta, beta_values, alpha_threshold_product, 'Beta Plots', 1, len(alpha_values), len(threshold_values))
#
# fig_threshold, axes_threshold = plt.subplots(len(alpha_values), len(beta_values), figsize=(15, 15))
# plot_subplots(fig_threshold, axes_threshold, threshold_values, alpha_beta_product, 'Threshold Plots', 2, len(alpha_values), len(beta_values))
#
# plt.show()
